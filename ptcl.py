...
def verify_file(stream: ReadableWriteableStream) -> bool:
    with SeekContext(stream):
        if (magic := stream.read(8).decode("utf-8")) != "VFXB    ":
            pass # print(f"Invalid magic: {magic}")
            return False
        stream.read_u8()
        if (version := stream.read_u8()) != 4:
            pass # print(f"Invalid graphics API version: {version}")
            return False
        if (version := stream.read_u16()) != 0x33:
            pass # print(f"Invalid resource version: {version}")
            return False
        if stream.read_u16() != 0xfeff:
            pass # print("Invalid endianness, only little endian is support")
            return False
        return True
...
def read_anim(stream: ReadableWriteableStream, count: int) -> List[AnimKeyFrame]:
    ...
    if count > 8:
        pass # print(f"Too many keyframes {count} @ {hex(stream.tell())}") # one of the files has it set to 8 and I'm not sure why?
        count = 7
    with SeekContext(stream):
...
def iter_emitters(stream: ReadableWriteableStream) -> Dict[str, Emitter]:
    ...
    while True:
        ...
        with SeekContext(stream):
            ...
            pass # print(f"    Emitter found: {name}")
...
def dump_json(path: str, output_path: str = "") -> Dict[str, Dict[str, Emitter]]:
    ...
    if path.endswith(".zs"):
        pass # print(f"Please decompress {path} first")
        return file
    if not os.path.exists(path):
        pass # print("File does not exist")
        return file
    with open(path, "rb+") as f:
        ...
        if (start := mm.find("VFXB    ".encode("utf-8"))) == -1:
            pass # print("Failed to find file magic")
            return file
        stream.seek(start)
        if not verify_file(stream):
            pass # print("File is not valid")
            return file
        seek_first_block(stream)
        if (offset := find_section(stream, "ESTA")) == 0xffffffff:
            pass # print("No emitter sets found")
            return file
        while True:
            stream.skip(offset)
            with SeekContext(stream):
    ...
    pass # print("Please note that the first keyframe for color and alpha animations is the second in the list and not the first")
    return file
...
def write_anim(stream: ReadableWriteableStream, frames: List[AnimKeyFrame]) -> None:
    with SeekContext(stream):
        ...
        if len(frames) > 8:
            pass # print("WARNING: Animation has too many key frames, some will be ignored (max of 8)")
    ...
    while True:
        ...
        with SeekContext(stream):
            ...
            with RelativeSeekContext(stream, read_header(stream)["section_offset"]):
                ...
                if name in changes:
                    pass # print(f"    Applying changes to {name}")
...
def apply_edits(ptcl_path: str, json_path: str) -> bool:
    if not os.path.exists(ptcl_path):
        pass # print("PTCL file does not exist")
        return False
    if ptcl_path.endswith(".zs"):
        pass # print(f"Please decompress {ptcl_path} first")
        return False
    if not os.path.exists(json_path):
        pass # print("JSON file does not exist")
        return False
    ...
    with open(ptcl_path, "rb+") as f:
        ...
        while True:
            ...
            with SeekContext(stream):
                ...
                if eset in changes:
                    pass # print(f"Applying changes to {eset}")
                    emitter_offset: int = read_header(stream)["subsection_offset"]
                    if emitter_offset == 0xffffffff:
                        pass # print(f"Skipping {eset}, no emitters found")
                    else:
                        with RelativeSeekContext(stream, emitter_offset):
...
def ptcl_apply_edits_lib(ptcl_data: bytes, json_data: bytes) -> str:
    ...
    changes: Dict[str, Dict[str, Emitter]] = yaml.load(json_data.decode('utf-8'), Loader=yaml.Loader)
